...
The encoders and scalers need no new code — the refit pipeline reuses
``fit_encoders`` and ``fit_scalers`` from data_engineering with a wider
``split_to_fit``. Only the model needs a dedicated node.
"""
