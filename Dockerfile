# Churn API — FastAPI over the Kedro pipelines.
#
...
# cheap layers instead of reinstalling catboost from scratch.
...
# without it the build of the `churn` package itself fails in phase 2.
...
CMD ["uv", "run", "uvicorn", "churn.api:app", "--host", "0.0.0.0", "--port", "8000"]
